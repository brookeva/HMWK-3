...
#how do we get the people weight and bike trips to interact in a way that makes sense? Just multiplying sum of person weight of bike by number of bike trips gives >77 million

### Amount of People who bike
sum(trip$P_WEIGHT[trip$MODE_AGG=='2'],na.rm=T)

### Number of bike trips
sum(trip$MODE_AGG=='2',na.rm=T)

##updated #3 LMK
round(table(trip$MODE_AGG[trip$P_WEIGHT])/length(trip$MODE_AGG[trip$P_WEIGHT]), 3)

sum(trip$P_WEIGHT)*.002

<<<<<<< HEAD
table(trip$MODE_AGG)/length(trip)
                         
=======
table(trip$MODE_AGG)/length(trip$MODE_AGG)

table(trip$MODE_AGG)/length(trip$MODE_AGG)*sum(trip$P_WEIGHT)
>>>>>>> b7503f4d3c4bdf5a1219e524eaa4871064e5c819
...
```{r}
#Average household income by mode choice?

hhtrip <- merge(trip, hh[c("HH_ID", "H_COUNTY", "INCOME")], by = "HH_ID")
hhtrip_fix <- filter(hhtrip, INCOME <"98")

hhtrip_summary <- hhtrip_fix %>% select(INCOME,MODE_AGG)

<<<<<<< HEAD
##table & plot with hhtrip_fix
table(hhtrip_fix$INCOME,hhtrip_fix$MODE_AGG)
<<<<<<< HEAD

plot(table(hhtrip_fix$INCOME,hhtrip_fix$MODE_AGG))

##table & plot with hhtrip
table(hhtrip$INCOME,hhtrip$MODE_AGG)
=======
table(hhtrip_fix$INCOME,hhtrip_fix$MODE_AGG)
>>>>>>> 070836072d6eca2362c88bdf8266d73a565146ef
  
## use rowbind or columnbind     
plot(table(hhtrip_fix$INCOME,hhtrip_fix$MODE_AGG))
=======
...
hhtrip <- merge(trip, hh[c("HH_ID", "H_COUNTY", "INCOME")], by = "HH_ID")
hhtrip_fix <- filter(hhtrip, INCOME <"98")
Q6Table <- table(hhtrip_fix$INCOME,hhtrip_fix$MODE_AGG)
